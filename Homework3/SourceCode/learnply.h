...
public:
	...
	void middlePoint( Vertex &p1 , Vertex &p2 ) {
		x = .5 * (p1.x + p2.x);
		y = .5 * (p1.y + p2.y);
		z = .5 * (p1.z + p2.z);
	}
...
public:
	...
	void append(Corner *c)
	{
		...
		/* first make sure there is enough room for new corners */
...
public:
	...
	void calc_bounding_box_withnormals();

	void loopsubdivision( );
	void ConstructCornerList();

	void MeshSmoothingUniformScheme();
	void MeshSmoothingCordScheme();
	void MeshSmoothingMeanCurvatureFlowScheme();
	void MeshSmoothingMeanValueScheme();

	void MeshSmoothingCordScheme_WithInitialMesh();
	void MeshSmoothingMeanCurvatureFlowScheme_WithInitialMesh();
	void MeshSmoothingMeanValueScheme_WithInitialMesh();

	void MorseDesignUsingExplicitMethod();
	void MorseDesignUsingGaussSeidalMethod();

	Polyhedron();
	Polyhedron(FILE *,int type =0);
